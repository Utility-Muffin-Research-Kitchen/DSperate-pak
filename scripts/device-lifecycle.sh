...
#
#   CONFIRM_DS_LIFECYCLE=1 scripts/device-lifecycle.sh
#
...
#   DS_ACCEPT_OUT         evidence dir (default: <repo>/build/lifecycle)
...
WORKSPACE="${LEAF_WORKSPACE_DIR:-$(cd "$REPO/.." && pwd)}"
LEAF="$WORKSPACE/Leaf"
JAWAKA="$WORKSPACE/Jawaka"
OUT="${DS_ACCEPT_OUT:-$REPO/build/lifecycle}"
PORT="${DS_PORT:-8765}"
...
STORE_VERSION="2.0.0"
FEED_ZIP="$REPO/build/dist/DSperate.mlp1.pak.zip"
...
make_feed() {
    [ -f "$FEED_ZIP" ] || (cd "$REPO" && make dist-pakrat >/dev/null)
    rm -rf "$FEED_DIR"
